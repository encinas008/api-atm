...
class PrescripcionRepresentante extends MY_Controller
{
    private $param;
    protected $baseurl;
    ...
    public function __construct()
    {
        ...
        !$this->atm = $this->load->database('atm', TRUE) ? $this->atm = $this->load->database('atm', TRUE) : false;
    ...
    */
    public function getPersonaRepreByCi_get($ci){
        header("Access-Control-Allow-Origin: *");
       
        if(!is_null($ci) && !empty($ci)){
            //echo "hola";
            $this->atm->select('*');
            $this->atm->from('persona');
            $this->atm->where('numero_documento', $ci);
            //$this->db_recaudaciones->where('estado', 'AC'); 
            $query = $this->atm->get();
       
            $persona_representante = $query->row();
               
            if(null !== $persona_representante){
                $this->response([
                    'status' => true,
