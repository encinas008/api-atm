...
class Prescripcion extends MY_Controller {
    ...
    public function __construct($config = 'rest')
	{
       ...
       // $this->load->model('DeclaracionJurada_model');
        ...
        //$this->load->model('presTipObjetoTriburario_model');PresPersonaJuridica_model
        $this->load->model('Prescripcion_model');
        $this->load->model('Persona_model');
        $this->load->model('PresSolicitante_model');
        $this->load->model('PresPersonaJuridica_model');
            ...
            {       
               ...
               // $persona['id_prescripcion'] = $insert_prescripcion->id;
                $persona = $this->Persona_model->create($persona);
                   //print_r($persona);
                    $pres_solicitante['id_persona'] = $persona->id;
                    $pres_solicitante= $this->PresSolicitante_model->create($pres_solicitante);
                   // print_r($persona->id);
                    $pres_prescripcion = $data["pres_prescripcion"];
                    $pres_prescripcion['id_solicitante']=$pres_solicitante->id;
                    $pres_prescripcion= $this->Prescripcion_model->create($pres_prescripcion);
                    // print_r($pres_solicitante);
                
                if( array_key_exists("representante_persona",  $data)){
                    $persona_representante = $data["representante_persona"];
                    $tipo_documento = $data['tipo_documento'];
                    print_r($tipo_documento);
                   
                     if( $tipo_documento == 'CI'){
                         $persona_representante = $this->Persona_model->create($persona_representante); 
                         $pres_representante_legal['id_solicitante'] = $pres_solicitante->id;
                         $pres_representante_legal['id_persona'] = $persona_representante->id; 
                         $pres_representante_legal = $this->PresRepresentanteLegal_model->create($pres_representante_legal); 
                        
                      }

                      if( $tipo_documento == 'NIT'){
                           array_splice($persona_representante, 0, 1);
                           $persona_representante = $this->PresPersonaJuridica_model->create($persona_representante); 
                           $pres_representante_legal['id_solicitante'] = $pres_solicitante->id;
                           $pres_representante_legal['id_persona_juridica'] = $persona_representante->id; 
                           $pres_representante_legal = $this->PresRepresentanteLegal_model->create($pres_representante_legal); 
                          print_r($insert_juridicos);
                      }
                    /*
                    }else{
                       echo "es vacio pres_representante_legal";
                       }
                       */
